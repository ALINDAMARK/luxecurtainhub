...
#hero-img {
    ...
    background:url('{{ $heroImageUrl ?? 'https://i.pinimg.com/736x/8f/ec/24/8fec2448ee910fb49370ffb4b1b3379e.jpg' }}') center/cover no-repeat;
...
#feat-img {
    position:absolute; inset:0;
    background:url('{{ $featuredImageUrl ?? 'https://i.pinimg.com/736x/58/9f/92/589f92fba7423b01456d998ff1718eb5.jpg' }}') center/cover no-repeat;
}
#feat-img::after {
    content:''; position:absolute; inset:0;
...
<!-- ═══════ PHOTO STRIP ═══════ -->
<div id="strip">
    @foreach ($stripImages as $image)
    <div class="strip-img">
        <img src="{{ $image['image_url'] }}" alt="{{ $image['alt_text'] }}" loading="lazy">
        <span>{{ $image['caption'] ?? $image['title'] }}</span>
    </div>
    @endforeach
</div>
...
<section id="lookbook" class="rev">
    ...
    <div class="lb-grid">
        ...
        <div class="lb-cell">
            <img src="{{ $image['image_url'] }}" alt="{{ $image['alt_text'] }}" loading="lazy">
            <div class="lb-label"><p>{{ $image['title'] }}</p><span>{{ $image['caption'] ?? 'LuxeCurtain Hub' }}</span></div>
        </div>
        @endforeach
    </div>
</section>
    ...
    <div class="card-grid">
        @foreach ($blogPosts as $post)
            <article class="card" style="padding:0;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.05);">
                <img src="{{ $post['image_url'] ?? 'https://images.unsplash.com/photo-1555041469-a586c61ea9bc?w=900&q=80' }}" alt="{{ $post['title'] }}" loading="lazy" style="width:100%;height:240px;object-fit:cover;display:block;">
                <div style="padding:1.25rem;">
                    <h3>{{ $post['title'] }}</h3>
                    <p style="color:var(--ash);line-height:1.8;">{{ $post['excerpt'] }}</p>
                </div>
            </article>
        @endforeach
    </div>
...
<section id="testimonials" class="rev">
    ...
    <div class="t-card">
        ...
        <div class="t-quote">"{{ $story['quote'] }}"</div>
        <div class="t-cite">{{ $story['client_name'] }} &nbsp;·&nbsp; {{ $story['location'] }}</div>
    </div>
    @endforeach
</section>
...
<section id="contact" class="rev">
    ...
    <form class="form" action="{{ route('consultation.store') }}" method="POST">
        @csrf
        <div class="fg">
            <label for="full_name">Full Name</label>
            <input id="full_name" name="full_name" type="text" placeholder="Your name" value="{{ old('full_name') }}">
            @error('full_name') <div style="margin-top:.45rem;color:#cf8d7d;font-size:.65rem;letter-spacing:.12em;text-transform:uppercase;">{{ $message }}</div> @enderror
        </div>
        <div class="fg">
            <label for="phone">Phone / WhatsApp</label>
            <input id="phone" name="phone" type="text" placeholder="+256 ..." value="{{ old('phone') }}">
            @error('phone') <div style="margin-top:.45rem;color:#cf8d7d;font-size:.65rem;letter-spacing:.12em;text-transform:uppercase;">{{ $message }}</div> @enderror
        </div>
        <div class="fg">
            <label for="email">Email</label>
            <input id="email" name="email" type="email" placeholder="your@email.com" value="{{ old('email') }}">
            @error('email') <div style="margin-top:.45rem;color:#cf8d7d;font-size:.65rem;letter-spacing:.12em;text-transform:uppercase;">{{ $message }}</div> @enderror
        </div>
        <div class="fg">
            ...
            @error('space_type') <div style="margin-top:.45rem;color:#cf8d7d;font-size:.65rem;letter-spacing:.12em;text-transform:uppercase;">{{ $message }}</div> @enderror
        </div>
        <div class="fg">
            ...
            @error('vision') <div style="margin-top:.45rem;color:#cf8d7d;font-size:.65rem;letter-spacing:.12em;text-transform:uppercase;">{{ $message }}</div> @enderror
        </div>
